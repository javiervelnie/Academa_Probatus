...
if (isset($_POST['login'])) {
    ...
    if ($email != "" || $password1 != "") {
        ...
        if ($email_check) {
            $consulta = $conn->prepare("SELECT * FROM academia.profesor WHERE email=:email");
            ...
            if ($existe) {
                $sql = $conn->prepare("SELECT * FROM academia.profesor WHERE email=:email AND password1=:password1");
                ...
                if ($existe) {
                    $sth = $conn->prepare("SELECT id FROM academia.profesor WHERE email=:email");
                    $sth->execute(array(':email' => $email));
                    $file = $sth->fetch();
                    $idConsulta = $file['id'];
                    $_SESSION['id'] = $idConsulta;
                    header('Location:corregirtareas.php');
                } else {
